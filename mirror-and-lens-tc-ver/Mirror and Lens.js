...
function drawLabels() {
    function writeText(xPos, yPos, color, text) {
        ...
        if (yPos <= principalAxis) {
            context.fillText(text, xPos*PpCM - 21, yPos*PpCM - 14);
            context.strokeText(text, xPos*PpCM - 21, yPos*PpCM - 14);
        } else {
            context.fillText(text, xPos*PpCM - 21, yPos*PpCM + 28);
            context.strokeText(text, xPos*PpCM - 21, yPos*PpCM + 28);
        }
    }

    writeText(equipments[0].x, equipments[0].y, '#ffffff', 'วัตถุ');
    if (imageToDisplay > images.length) {
        imageToDisplay = images.length;
    }
    if (images.length == 1) {
        writeText(images[0].x, images[0].y, colorCycle[0], 'ภาพ');
    } else {
        if (imageToDisplay == 0) {
            for (let i = 0; i < images.length; i++) {
                writeText(images[i].x, images[i].y, colorCycle[i % (colorCycle.length)], 'ภาพ ' + (i + 1));
            }
        } else {
            writeText(images[imageToDisplay - 1].x, images[imageToDisplay - 1].y, colorCycle[(imageToDisplay - 1) % (colorCycle.length)], 'ภาพ ' + imageToDisplay);
            if (imageToDisplay >= 2) {
                writeText(images[imageToDisplay - 2].x, images[imageToDisplay - 2].y, colorCycle[(imageToDisplay - 2) % (colorCycle.length)], 'ภาพ ' + (imageToDisplay - 1));
            }
        }
    }
}